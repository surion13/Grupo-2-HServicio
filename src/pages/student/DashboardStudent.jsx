...
export default function DashboardStudent() {
  ...
  // Unificamos el ciclo de vida en un único efecto controlado para evitar renders infinitos
  useEffect(() => {
    let isMounted = true;
    ...
    const cargarDatosDashboard = async () => {
      try {
        if (isMounted) {
          // Ejecuta ambas peticiones en paralelo de forma segura
          await Promise.all([
            fetchStats(),
    ...
    return () => {
      isMounted = false; // Desactiva actualizaciones si el componente se desmonta de verdad
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, pageSize]); // Se dispara únicamente cuando el usuario cambia de página o de cantidad de ítems
  ...
  const handlePageSizeChange = (e) => {
    ...
    setPageSize(newSize);
    setPage(1); // Resetea a la página 1 para evitar desbordamientos
  };
